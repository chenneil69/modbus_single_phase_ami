...
const PRE = 'single_phase';

export async function insertAmi(station: number, readings: any) {
	let modelName = '';
	if (station === 9) {
		modelName = `${PRE}_ami_${dayjs().format('YYYY_MM')}`;
	} else {
		modelName = station === 0 ? `${PRE}_ami_${dayjs().format('YYYY_MM')}` : `${PRE}_branch_${station}_ami_${dayjs().format('YYYY_MM')}`;
	}
